...
#ifdef CONFIG_KSU
	/*
	 * Manual/legacy hook build: no kprobes exist to ever flip
	 * ksu_execveat_hook true, so the conditional dispatch to
	 * ksu_handle_execveat_sucompat() left the manager's allowlist
	 * grant/deny path permanently dead. Always invoke the full
	 * handler instead.
	 */
